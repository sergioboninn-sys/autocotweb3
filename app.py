...
# --- ABA 1: COTAÇÃO (CORREÇÃO DA CONTAGEM) ---
if aba == "📊 Cotação":
    ...
    if target_file:
        ...
        if st.button("🚀 Processar e Preservar Formatação"):
            ...
            try:
                ...
                count = 0 # Reiniciando o contador

                for r in range(int(start_row), ws.max_row + 1):
                    d_val = ws.cell(row=r, column=d_idx).value
                    # Se não tem descrição, ignoramos a linha (evita rodapés vazios)
                    ...
                    # 3. Gravação e incremento do contador REAL
                    if found_p is not None:
                        ...
                        count += 1 # SÓ INCREMENTA SE ACHOU O PREÇO
